...
void drawElevatorScreen(int floor, ElevatorState_t state)
{
    ...
    int textTotalWidth = (charLen * bitmapWidth) + ((charLen - 1) * textGap);

    int16_t floorStartX = (SSD1306_WIDTH - textTotalWidth) / 2;
    int16_t contentY = (SSD1306_HEIGHT - bitmapHeight) / 2;

    int16_t currentX = floorStartX;
    ...
    for (int i = 0; i < charLen; i++)
    {
        ...
        currentX += bitmapWidth + textGap;
    }


    if (state == STATE_MOVING_UP)
    {
        int16_t arrowX = floorStartX - bitmapWidth - arrowGap;
        ssd1306DrawBitmap( arrowX, contentY, arrowUpBitmap, bitmapWidth, bitmapHeight, SSD1306_COLOR_WHITE );
    }


    else if (state == STATE_MOVING_DOWN)
    {
        int16_t arrowX = floorStartX + textTotalWidth + arrowGap;
        ssd1306DrawBitmap( arrowX, contentY, arrowDownBitmap, bitmapWidth, bitmapHeight, SSD1306_COLOR_WHITE );
    }
